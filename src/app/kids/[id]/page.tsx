...
const selectedCategories = useMemo(() => ["Older Kids' Shoe", "Older Kids' Shoes"]
, [] 
);
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const query = `*[_type == "products" && category in $categories && _id == $id][0]{
          _id, name, "imageUrl": image.asset->url, description, price, sizes, rating,
          stock, discount, category, color, details, style, tag
        }`;

        const fetchedProduct: Product = await client.fetch(query, { id, categories: selectedCategories });
  ...
  if (!product) {
    return <div className="text-center text-lg font-bold text-red-500">Product not found!</div>;
  }
