...
const ProductDetails = ({ params }: CategoryDetailsProps) => {
  const { id } = params;
  ...
  useEffect(() => {
    const fetchNikeAirMax = async () => {
      try {
        ...
        const fetchedCategory: Category = await client.fetch(query, { id });
        setCategory(fetchedCategory);
      } catch (error) {
        console.error("Error fetching Nike Air Max Products:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAddToBag = () => {
    ...
    toast.success(`${category.name} is added to bag!`);
  ...
  const handleAddToWishlist = () => {
    ...
    toast.success(`${category.name} is added to wishlist!`);
  };
        ...
        <div className="max-w-[1200px] w-full h-[580px] grid grid-cols-1 lg:grid-cols-2 xs:w-[400px] sm:w-[700px]">
          ...
          <div className="w-[376px] h-auto grid grid-cols-1 lg:ml-[400px] md:mt-6 xs:mt-2">
            <div className="flex flex-col">
              <p className="text-gray-900 text-2xl font-semibold">{category.name}</p>
              <p className="text-gray-900 text-[16px] font-semibold">{category.category}</p>
              <p className="text-[16px] font-semibold mt-4">MRP: ₹{category.price.toFixed(2)}</p>
              ...
              <div className="mt-10 ml-8 xs:ml-14 text-sm font-medium text-gray-900 gap-2 text-center md:text-left md:mr-0 mr-28 xxs:mr-10 xxs:w-[220px]">
                <p className="text-gray-900">Color Shown: {category.color}</p>
                <p className="text-gray-900">{category.details}</p>
                <p className="text-gray-900">{category.style}</p>
              </div>
              <div>
